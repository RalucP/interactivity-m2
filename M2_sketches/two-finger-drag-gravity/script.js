...
const use = () => {
    ...
    if (thingIntervalXMin < centroid.last.x &&
        ...
        Math.abs(distance) < heightTreshold) {
            ...
            const halfWidth = parseInt(/** @type string */(thing?.el.style.width)) / 2;

            // Update the thing in state.things according to its id
            finalThing = updateThingInState({ position: updatedPosition, agitation: (offsetDistance / halfWidth), transformOrigin: centroid.last});           
...
const update = () => {
    ...
    if (byAge.length === 2) {
        ...
        updateThingInState({ agitation: 0});           
    }
