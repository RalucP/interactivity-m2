...
const onPointerDown = event => {
  ...
  const relativePointerPos = {
    x: pointerRelative.x,
    y: pointerRelative.y - 0.3,
  }
 
  // Init new 'relation', and update state
  saveState({ relationFromPointerDown: Points.relation(relativePointerPos) , pointerPositionStart: relativePointerPos});

  // Position 'reference' element to pointer location
  Util.positionIdByRelative(`reference`, relativePointerPos);
